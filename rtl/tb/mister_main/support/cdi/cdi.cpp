...
static void toc_data(int lba, struct read_toc& out)
{
	if (lba < 0)
	{
        ...
        while (toc_ptr < toc_buffer.data() + toc_entry_count &&
            ...
            if (0) /* HACK */ {
                if (toc_ptr->track == 2)
                    out.track_desc[desc_idx].mins = 0x01;
                    out.track_desc[desc_idx].secs = 0x00;
                    out.track_desc[desc_idx].frac = 0x45 - 24;
            }
            toc_ptr++;
            desc_idx++;
        }
...
void cdi_read_cd(uint8_t* buffer, int lba, int cnt)
{
	...
	while (cnt > 0)
	{
		if (lba < 0 || !toc.last)
		{
            ...
            if (0) {
                size_t len = ntohs(toc_out.length);
