...
__int64_t FileGetSize(fileTYPE *file)
{
	if (file->filp)
	{
		struct stat st;
...
int FileSeek(fileTYPE *file, __int64_t offset, int origin)
{
	if (file->filp)
	{
		__int64_t res = fseeko(file->filp, offset, origin);
		if (res < 0)
		{
	...
	else if (file->zip)
	{
		...
		{
			const size_t want_len = MIN((__int64_t)sizeof(buf), offset - file->zip->offset);
...
{
	__int64_t int64 = offset;
	int64 <<= 9;
	return FileSeek(file, int64, SEEK_SET);
}
