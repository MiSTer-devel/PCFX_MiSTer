// Return To Zork, Town Hall, filing cabinet, lower drawer

task load_vce_reg();
    io_sel = VCE;
    reg_write(7'h00, 16'h4f00); // CR
    ...
    reg_write(7'h10, 16'h0888); // BL1A
    reg_write(7'h11, 16'h0000); // BL1B
...
task load_kreg();
    io_sel = MMC;
    reg_write(7'h0f, 16'h1101); // PAGESET
    // KING BG
    ...
    // RAINBOW
    reg32_write(7'h41, 32'h00034000); // RKRAMA
...
task load_vmem();
`ifdef TB_VDC
    $readmemh("vram0-rtz5.hex", vram0.mem);
    $readmemh("vram1-rtz5.hex", vram1.mem);
`endif
    $readmemh("vce_cp-rtz5.hex", vce.cpram.mem);
    vram_load_file("kram0-rtz5.bin", 0);
    vram_load_file("kram1-rtz5.bin", 1);
endtask
