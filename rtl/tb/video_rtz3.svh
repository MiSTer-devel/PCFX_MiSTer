// Return To Zork, last video frame ascending lighthouse stairs

task load_vce_reg();
    io_sel = VCE;
    reg_write(7'h00, 16'h5f00); // CR
...
task load_vdc0_reg();
    ...
    reg_write(7'h13, 16'hfc00); // DVSSR
endtask
...
task load_vdc1_reg();
    ...
    reg_write(7'h06, 16'h0000); // RCR
    reg_write(7'h07, 16'h03d3); // BXR
    reg_write(7'h08, 16'h0168); // BYR
    reg_write(7'h09, 16'h0000); // MWR
    reg_write(7'h0a, 16'h0202); // HSR
    reg_write(7'h0b, 16'h041f); // HDR
    reg_write(7'h0c, 16'h1102); // VPR
...
task load_kreg();
    io_sel = MMC;
    reg_write(7'h0f, 16'h1100); // PAGESET
    // KING BG
...
task load_rreg();
    ...
    io_write16(4'h2, 16'h0001); // (R01) {FTS, CHR, SRT}
    io_write16(4'h4, 16'h0080); // (R02) NRY
...
task load_vmem();
`ifdef TB_VDC
    $readmemh("vram0-rtz3.hex", vram0.mem);
    $readmemh("vram1-rtz3.hex", vram1.mem);
`endif
    $readmemh("vce_cp-rtz3.hex", vce.cpram.mem);
    vram_load_file("kram0-rtz3.bin", 0);
    vram_load_file("kram1-rtz3.bin", 1);
endtask
